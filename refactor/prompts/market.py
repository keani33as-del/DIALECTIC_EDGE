...
Если есть явная сделка:
• Символ | Направление | Вход | Стоп | Цель | R/R | Размер | Горизонт

Если НЕТ явной сделки (NEUTRAL):
• Обязательно укажи ТОЧКИ НАБЛЮДЕНИЯ — конкретные уровни цен для входа
• Триггеры: "Ждём пробоя $X" или "Вход при откате к $Y"
...
💼 ТОРГОВЫЙ ПЛАН:
[Формат если есть сделка:]
• [Актив] | [LONG/SHORT/CASH] | Вход: $[цена] | Стоп: $[цена] (+-[X]%) | Цель: $[цена] (+-[Y]%) | R/R 1:[N] | Размер: [P]% | Горизонт: [срок]

[Формат если НЕТ сделки - ОБЯЗАТЕЛЬНО заполни:]
• CASH (основная) | FinBERT=[нейтральный/бычий/медвежий] | VIX=[значение]
  👀 Триггер LONG: [актив] пробой $[уровень] → LONG [P]%
  👀 Триггер SHORT: [актив] откат к $[уровень] → SHORT [P]%
